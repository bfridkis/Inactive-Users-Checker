...
if (!$helpRequest) {
    ...
    if ($outputMode -eq 1) { 
        ...
        $userInfo | select-object @{n='Name';e={$_.Name}}, lastLogonDate, 
                        @{n = 'Number of Days Inactive' ; e = { (New-TimeSpan -Start $_.lastLogonDate -End $checkDate).Days }
                        } | 
         where-object {($_.lastLogonDate -lt $checkDate.adddays(-100))} |

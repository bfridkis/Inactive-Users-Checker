...
if (!$helpRequest) {
    write-output "`n"
    write-output "`t`t  *$*$*  Inactive Users Checker *$*$*`n"
    ...
    $defaultOutFileName = "InactiveUsersCheckerOut-$(Get-Date -Format MMddyyyy_HHmmss)"
    ...
    if ($outputMode -eq 1) { 
        if ($OUSpecific -eq "N") { get-aduser -Filter "Enabled -eq 'True'" -Property lastLogonDate -SearchScope Subtree | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
        else { get-aduser -Filter "Enabled -eq 'True'" -SearchBase $OUToSearch -Property lastLogonDate -SearchScope $searchLevel | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
        $userInfo | select-object @{n='Name';e={$_.Name}},
                        @{n = 'lastLogonDate' ; e =  { if($_.lastLogonDate) { $_.lastLogonDate } else { "NONE" } }},
                        @{n = 'Number of Days Inactive' ; e = { if($_.lastLogonDate) { (New-TimeSpan -Start $_.lastLogonDate -End $checkDate).Days }
                                                                else { "N/A"} }
                        } | 
         where-object {($_.lastLogonDate -lt $checkDate.adddays(-100) -or $_.lastLogonDate -eq "NONE")} |
         sort-object -Descending 'Number of Days Inactive' -OutVariable Export *>$null
    }
    else {
        if ($OUSpecific -eq "N") { get-aduser -Filter "Enabled -eq 'True'" -Property lastLogonDate -SearchScope Subtree | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
        else { get-aduser -Filter "Enabled -eq 'True'" -SearchBase $OUToSearch -Property lastLogonDate -SearchScope $searchLevel | Tee-Object -Variable userInfo | Measure -OutVariable userCount >$null }
        $userInfo | select-object @{n='Name';e={$_.Name}}, 
         ...
         where-object {($_.lastLogonDate -lt $checkDate.adddays(-100) -or $_.lastLogonDate -eq "NONE")} |
         sort-object -Descending 'Number of Days Inactive' -OutVariable Export | Format-Table -autosize
    }
    ...
    write-output "`n"
    write-output "`t`t`t`t`t`t`t`t`t*!*!* Inactive Users Checker - Help Page*!*!*"

    write-output "SYNTAX"
    write-output "`tInactiveUsersCheckerV1.0.ps1 [-CheckComplianceAsOfDate <date> {format: mm/dd/yyyy}]"
